...
-- Xác định số lượng dự án mà mỗi công ty tham gia có trạng thái 'Đang thực hiện'. Chỉ bao gồm các công ty có hơn một dự án đang thực hiện. 
-- Sắp xếp kết quả theo số lượng dự án giảm dần.
SELECT MaCongTy, COUNT(*)
FROM DuAn
WHERE TrangThai = N'Đang thực hiện' 
GROUP BY MaCongTy
HAVING COUNT(*) > 1
...
SELECT TenCongTy, AVG(SoNhanVien)
FROM CongTy
WHERE SoNhanVien BETWEEN 50 AND 150
GROUP BY TenCongTy
ORDER BY TenCongTy

-- Xác định số lượng kỹ năng cho mỗi chuyên gia có cấp độ tối đa là 5 và chỉ bao gồm những chuyên gia 
-- có ít nhất một kỹ năng đạt cấp độ tối đa này. Sắp xếp kết quả theo tên chuyên gia tăng dần.
...
-- Chỉ bao gồm những kỹ năng có tổng số chuyên gia lớn hơn 2. Sắp xếp kết quả theo tên kỹ năng tăng dần.
SELECT TenKyNang, COUNT(MaChuyenGia)
FROM ChuyenGia_KyNang JOIN KyNang
ON ChuyenGia_KyNang.MaKyNang = KyNang.MaKyNang
WHERE CapDo >= 4
GROUP BY TenKyNang
...
SELECT HoTen, AVG(CapDo)
FROM ChuyenGia JOIN ChuyenGia_KyNang
